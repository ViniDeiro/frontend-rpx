...
import { Eye, EyeOff, Lock, Mail, AlertTriangle, LogIn } from 'react-feather';
import { useAuth } from '@/contexts/AuthContext';

export default function LoginPage() {
  const router = useRouter();
  const { login, isSimulatedMode, toggleSimulatedMode, loginAsDemoUser } = useAuth();
  ...
  // Alternar o modo simulado
  const handleToggleSimulatedMode = () => {
    toggleSimulatedMode();
    setError('');
  };

  // Entrar diretamente sem login (modo visitante)
  const handleVisitorLogin = () => {
    if (!isSimulatedMode) {
      toggleSimulatedMode(); // Ativa o modo simulado se não estiver ativo
    }
    loginAsDemoUser(); // Loga diretamente com o usuário demo
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="bg-card-bg border border-gray-700 max-w-md w-full space-y-8 p-8 rounded-xl shadow-lg">
        ...
        {/* Botão para acessar diretamente sem login */}
        <button
          type="button"
          onClick={handleVisitorLogin}
          className="w-full flex justify-center items-center gap-2 py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-md"
        >
          <LogIn size={20} />
          <span>Entrar Diretamente (Modo Simulado)</span>
        </button>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-card-bg text-gray-400">ou use suas credenciais</span>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex justify-center py-2 px-4 border border-purple-500/30 rounded-lg shadow-sm text-purple-400 bg-transparent hover:bg-purple-500/10 focus:outline-none"
              >
                Login demo
              </button>
              ...
              <button
                type="button"
                onClick={handleToggleSimulatedMode}
                className={`flex-1 flex justify-center py-2 px-4 border rounded-lg shadow-sm focus:outline-none ${
                  isSimulatedMode 
                    ? 'border-amber-500/30 text-amber-400 bg-transparent hover:bg-amber-500/10' 
                    : 'border-green-500/30 text-green-400 bg-transparent hover:bg-green-500/10'
                }`}
              >
                {isSimulatedMode ? 'Desativar Simulação' : 'Ativar Simulação'}
              </button>
            </div>
          </div>
