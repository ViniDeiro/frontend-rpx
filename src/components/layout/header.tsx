...
import { Menu, X, User, DollarSign, ChevronDown, AlertTriangle, LogIn } from 'react-feather';
...
export const Header = () => {
  ...
  const { user, isAuthenticated, logout, isSimulatedMode, toggleSimulatedMode, loginAsDemoUser } = useAuth();
  ...
  return (
    ...
    >
      {isSimulatedMode && (
        <div className="bg-amber-500/10 w-full py-1 text-center">
          <div className="container mx-auto px-4 flex items-center justify-center gap-2">
            <AlertTriangle className="text-amber-400" size={16} />
            <span className="text-amber-400 text-xs font-medium">Modo Simulado Ativo</span>
          </div>
        </div>
      )}
                ...
                <div className="relative">
                  ...
                  >
                    <div className="relative w-8 h-8 overflow-hidden rounded-full">
                      <OptimizedImage 
                        src={user.avatarId ? `/images/avatars/${user.avatarId}.png` : ImagePaths.avatarPlaceholder} 
                        alt={user.name}
                        width={32}
                        height={32}
                  ...
                  {isProfileOpen && (
                    ...
                    >
                      ...
                      <button 
                        ...
                        className={`w-full text-left px-4 py-2 text-sm flex items-center gap-2 hover:bg-card-hover ${
                          isSimulatedMode ? 'text-amber-400' : 'text-green-400'
                        }`}
                      >
            ...
            <nav className="flex flex-col space-y-2">
              ...
              {isAuthenticated && user ? (
                <>
                  ...
                  {/* Botão Modo Simulado - Mobile */}
                  <button 
                    onClick={() => {
                      handleToggleSimulatedMode();
                      setIsOpen(false);
                    }}
                    className={`px-4 py-3 text-left rounded-md text-base font-medium hover:bg-card-hover flex items-center ${
                      isSimulatedMode ? 'text-amber-400' : 'text-green-400'
                    }`}
                  >
                ...
                <div className="flex flex-col space-y-2 px-4 py-2">
                  <button
                    ...
                    className="w-full flex justify-center items-center gap-2 py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-sm"
                  >
                    <LogIn size={16} />
                    <span>Entrar Demo</span>
                  </button>
